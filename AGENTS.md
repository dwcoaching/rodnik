...
- Use Herd for PHP and Composer commands so they run with the site's configured PHP version: `herd php ...`, `herd php artisan ...`, and `herd composer ...`. Do not rely on bare `php` or `composer` for verification.

### Testing
- `herd php artisan test` - Run tests using Pest (configured in phpunit.xml)
